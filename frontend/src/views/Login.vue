<template>
  <div class="login-page">
    ...
    <div class="login-card">
      ...
      <p class="subtitle">时间管理 · 登录</p>
      <form @submit.prevent="handleLogin" novalidate>
        <div class="form-group">
          <label class="field-label">邮箱</label>
          <div class="input-wrap">
            <svg class="input-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"/><polyline points="22,6 12,13 2,6"/></svg>
            <input v-model="email" type="text" placeholder="邮箱" autocomplete="email" required />
          </div>
        </div>
        <div class="form-group">
          ...
          <div class="password-wrap">
            ...
            <input v-model="password" :type="showPassword ? 'text' : 'password'" placeholder="密码" required />
        ...
        <button type="submit" class="btn-primary" :disabled="!canSubmit">登 录</button>
      </form>
      <div class="links">
...
const canSubmit = computed(() => {
  return email.value?.trim() && password.value && verifyCode.value?.trim()
})
...
async function handleLogin() {
  error.value = ''
  try {
    const res = await authApi.login({ email: email.value, password: password.value, verifyCode: verifyCode.value })
